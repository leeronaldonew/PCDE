function [RMSE_Info,k_True,Ki_True,Vd_True,Excel,Bias_Excel,SD_Excel,NBias_Excel,NSD_Excel,MSE_Excel,NBias_micro_2TCM,NBias_micro_PCDE,NBias_macro_2TCM,NBias_macro_PCDE,NBias_macro_Patlak,NSD_micro_2TCM,NSD_micro_PCDE,NSD_macro_2TCM,NSD_macro_PCDE,NSD_macro_Patlak,MSE_micro_2TCM,MSE_micro_PCDE,MSE_macro_2TCM,MSE_macro_PCDE,MSE_macro_Patlak]=virtual_simul_voxel_LSTM(scale_factor,Num_real, org_ind)
...
%% for Selecting Rough Range
ranges=combinator(5,3,'p', 'r');
% for K1
ranges(find(ranges(:,1)==1),1)=0.1;ranges(find(ranges(:,1)==2),1)=0.3;ranges(find(ranges(:,1)==3),1)=0.5;ranges(find(ranges(:,1)==4),1)=0.7;ranges(find(ranges(:,1)==5),1)=0.9;
% for k2
ranges(find(ranges(:,2)==1),2)=0.1;ranges(find(ranges(:,2)==2),2)=0.3;ranges(find(ranges(:,2)==3),2)=0.5;ranges(find(ranges(:,2)==4),2)=0.7;ranges(find(ranges(:,2)==5),2)=0.9;
% for k3
ranges(find(ranges(:,3)==1),3)=0.12;ranges(find(ranges(:,3)==2),3)=0.37;ranges(find(ranges(:,3)>=3),:)=[];
% for k4
ranges(:,4)=0; % for FDG, k4=0;
...
%[permutes,true_database]=make_database_NH_FDG_Full_to_5(PI_Time);

% True & Noisy data
%Num_real=100;
Ct_True=TTCM_analytic_Multi(k_True,PI_Time); % with Exp_4
Ct_True_2TCM=TTCM_analytic_Multi(k_True,[0:1:90]); % 
%Ct_True=transpose(TTCM_vector(k_True,transpose(PI_Time))); % with Feng's model
...
for i=1:1:Num_real
    ...
    ind_Range=double(classify(net,Fitted_Noisy));
    Range_temp_k1=[ranges(ind_Range,1)-0.1, ranges(ind_Range,1)+0.1];
    Range_temp_k2=[ranges(ind_Range,2)-0.1, ranges(ind_Range,2)+0.1];
    if ranges(ind_Range,3)==0.12
        Range_temp_k3=[0, 0.25];
    else
        Range_temp_k3=[0.25, 0.5];
    end
    permutes_temp(find(permutes_temp(:,1) < Range_temp_k1(1) | permutes_temp(:,1) > Range_temp_k1(2)),:)=[]; % for K1
    permutes_temp(find(permutes_temp(:,2) < Range_temp_k2(1) | permutes_temp(:,2) > Range_temp_k2(2)),:)=[]; % for k2
    permutes_temp(find(permutes_temp(:,3) < Range_temp_k3(1) | permutes_temp(:,3) > Range_temp_k3(2)),:)=[]; % for k3
    
    Y_data_repmat=repmat(Ct_Noisy(i,:), size(permutes_temp,1),1);
    true_database_temp=TTCM_analytic_Multi(permutes_temp,PI_Time);
    [sort_val,sort_ind]=sort( sum(abs(true_database_temp-Y_data_repmat) , 2) ); % 
    [ind_min]=Similarity_Measure2(permutes_temp, sort_ind, Ct_Noisy(i,:), PI_Time);
    if i==1
        chosen_params=permutes_temp(ind_min,:);
    else
        chosen_params=cat(1, chosen_params, permutes_temp(ind_min,:));
    end
    %% Original Algorithm
    ...
    %[ind_min]=Similarity_Measure2(permutes, sort_ind, Ct_Noisy(i,:), PI_Time);
...
for i=1:1:Num_real
    ...
    %% Time Series Classification with Deep Learning
    permutes_temp=permutes;
    ind_Range=double(classify(net,Ct_Denoisy(i,:)));
    Range_temp_k1=[ranges(ind_Range,1)-0.1, ranges(ind_Range,1)+0.1];
    Range_temp_k2=[ranges(ind_Range,2)-0.1, ranges(ind_Range,2)+0.1];
    if ranges(ind_Range,3)==0.12
        Range_temp_k3=[0, 0.25];
    ...
    Y_data_repmat=repmat(Ct_Denoisy(i,:), size(permutes_temp,1),1);
    true_database_temp=TTCM_analytic_Multi(permutes_temp,PI_Time);
    [sort_val,sort_ind]=sort( sum(abs(true_database_temp-Y_data_repmat) , 2) ); % 
    [ind_min]=Similarity_Measure2(permutes_temp, sort_ind, Ct_Denoisy(i,:), PI_Time);
    if i==1
        chosen_params=permutes_temp(ind_min,:);
    else
        chosen_params=cat(1, chosen_params, permutes_temp(ind_min,:));
    ...
    %[ind_min]=Similarity_Measure2(permutes, sort_ind, Ct_Denoisy(i,:), PI_Time);
...
MSE_Excel=[MSE_micro_2TCM;MSE_macro_2TCM;MSE_micro_PCDE;MSE_macro_PCDE;MSE_micro_PCDE_FLT;MSE_macro_PCDE_FLT;MSE_macro_Patlak];
...
Excel=[MSE_Excel,NBias_Excel,NSD_Excel,Mean_Pred_Excel];
