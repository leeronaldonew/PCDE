...
%simSet.dwellTime      = diff(frame);    % Frame lengths. (original one)
simSet.dwellTime      = 45+zeros(size(frame,1)-1,size(frame,2));    % Frame lengths. (modified by KNLEE to make a scan duration for a single frame 45 [sec])
...
%simSet.halflife       = 109.771*60;     % Halflife of nuclide in sec, or 'none' for no decay. (18-F)
simSet.halflife       = 'none';
...
simSet.radBin     = 283;        % Number of radial bins.
...
simSet.zFilter    = [1 2 1]/4;  % post recon Z-axis filter 3-point smoothing
                                % Heavy[ 1 2 1]/4, Standard[1 4 1]/6, Light[1 6 1]/8, None[0 1 0]
simSet.postFilter = 8;          % FWHM in (mm) of post reconstruction filter. FWHM = 2*sqrt(2*log(2))*sigma.

%simSet.zFilter    = [0 1 0]; % none
%simSet.postFilter = 0; % none 


simSet.iterNUM    = 5;          % number of iterations
simSet.subNUM     = 24;         % number of subsets
